...
const parentManifestPath = 'docs/freeze/panel-global-new-messages-v44-20260822.json';
const parentManifestSha256 = '239795973a82cec2beea290e449298641b42bb8a4e0627e7a352e71509cb97c5';
const manifestPath = 'docs/freeze/ec-delivered-repurchase-v45-20260822.json';
const lineageSpecs = [
    ...
    [parentManifestPath, 'panel-global-new-messages-v44-20260822', parentManifestSha256]
...
const newProtectedFiles = [
    ...
    'docs/EC_DELIVERED_REPURCHASE_FREEZE_V45_20260822.md',
    'scripts/assert-ec-delivered-repurchase-activation-approved-v45.mjs',
...
for (const spec of lineageSpecs) {
    if (!exists(spec.path) || sha256(spec.path) !== spec.sha256) {
        throw new Error(`[EC-DELIVERED-REPURCHASE-V45] manifesto ancestral divergente: ${spec.path}`);
    }
}
if (!exists(manifestPath)) {
    throw new Error('[EC-DELIVERED-REPURCHASE-V45] manifesto V45 ausente; startup bloqueado.');
}
...
if (
    parent.freezeId !== 'panel-global-new-messages-v44-20260822'
    || manifest.freezeId !== 'ec-delivered-repurchase-v45-20260822'
    ...
    || manifest.operatorApproval?.scope !== 'delivered_customer_new_repurchase_order_v45'
    || manifest.policy?.previousOrderImmutable !== true
    || manifest.policy?.newOrderPrefix !== 'EC-RECOMPRA-'
    || manifest.policy?.panelAuthRequired !== true
    || manifest.policy?.samePhoneRequired !== true
    || manifest.policy?.deliveryEvidenceRequired !== true
    ...
    throw new Error('[EC-DELIVERED-REPURCHASE-V45] manifesto ou política inválida; startup bloqueado.');
}
...
for (let index = 0; index < lineage.length; index += 1) {
    ...
    if (current.freezeId !== spec.freezeId) {
        throw new Error(`[EC-DELIVERED-REPURCHASE-V45] freeze ancestral inválido: ${spec.path}`);
    }
    if (index > 0 && current.parentFreezeId !== lineage[index - 1].freezeId) {
        throw new Error('[EC-DELIVERED-REPURCHASE-V45] linhagem V28 → V44 divergente.');
    }
    ...
    for (const [relativePath, approvedHash] of Object.entries(current.protectedFiles || {})) {
        ...
        if (!exists(relativePath) || sha256(relativePath) !== approvedHash) {
            throw new Error(`[EC-DELIVERED-REPURCHASE-V45] herança divergente em ${relativePath}.`);
        }
    }
}
...
for (const [relativePath, approvedHash] of Object.entries(manifest.protectedFiles || {})) {
    if (!exists(relativePath) || sha256(relativePath) !== approvedHash) {
        throw new Error(`[EC-DELIVERED-REPURCHASE-V45] alteração não autorizada em ${relativePath}.`);
    }
}

console.log('[EC-DELIVERED-REPURCHASE-V45] pedido entregue preservado e nova recompra autenticada ativos.');
