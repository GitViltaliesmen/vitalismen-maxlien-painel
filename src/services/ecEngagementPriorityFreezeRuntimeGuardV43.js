...
const parentManifestPath = 'docs/freeze/ec-engagement-command-reply-v42-20260822.json';
const parentManifestSha256 = '3dfb04a48226b960f42d59829c347b4ba83a45876039cc8020b4fa761386611d';
const manifestPath = 'docs/freeze/ec-engagement-priority-v43-20260822.json';
const lineageSpecs = [
    ...
    [parentManifestPath, 'ec-engagement-command-reply-v42-20260822', parentManifestSha256]
...
const newProtectedFiles = [
    'approved_freezes/APPROVED_EC_ENGAGEMENT_PRIORITY_V43_20260822.txt',
    'docs/EC_ENGAGEMENT_PRIORITY_FREEZE_V43_20260822.md',
    'public/panel-intelligence/ec-engagement-priority-v43.js',
    'scripts/assert-ec-engagement-priority-activation-approved-v43.mjs',
    'scripts/guard-ec-engagement-priority-v43.mjs',
    'src/services/ecEngagementPriorityFreezeRuntimeGuardV43.js',
    'tests/ec-engagement-priority-v43.test.mjs'
];
...
for (const spec of lineageSpecs) {
    if (!exists(spec.path) || sha256(spec.path) !== spec.sha256) {
        throw new Error(`[EC-ENGAGEMENT-PRIORITY-V43] manifesto ancestral divergente: ${spec.path}`);
    }
}
if (!exists(manifestPath)) {
    throw new Error('[EC-ENGAGEMENT-PRIORITY-V43] manifesto V43 ausente; startup bloqueado.');
}
...
if (
    parent.freezeId !== 'ec-engagement-command-reply-v42-20260822'
    || manifest.freezeId !== 'ec-engagement-priority-v43-20260822'
    ...
    || manifest.operatorApproval?.scope !== 'ec_engagement_priority_v43'
    || manifest.policy?.defaultChatFilter !== 'all'
    || manifest.policy?.engagementExcludedFromNewMessages !== true
    || manifest.policy?.engagementOwnUnreadBadge !== true
    || JSON.stringify(manifest.policy?.passiveBatchPattern) !== JSON.stringify([2, 3])
    || manifest.policy?.passiveBatchText !== '👍'
    ...
    throw new Error('[EC-ENGAGEMENT-PRIORITY-V43] manifesto ou política inválida; startup bloqueado.');
}
...
for (let index = 0; index < lineage.length; index += 1) {
    ...
    if (current.freezeId !== spec.freezeId) {
        throw new Error(`[EC-ENGAGEMENT-PRIORITY-V43] freeze ancestral inválido: ${spec.path}`);
    }
    if (index > 0 && current.parentFreezeId !== lineage[index - 1].freezeId) {
        throw new Error('[EC-ENGAGEMENT-PRIORITY-V43] linhagem V28 → V42 divergente.');
    }
    ...
    for (const [relativePath, approvedHash] of Object.entries(current.protectedFiles || {})) {
        ...
        if (!exists(relativePath) || sha256(relativePath) !== approvedHash) {
            throw new Error(`[EC-ENGAGEMENT-PRIORITY-V43] herança divergente em ${relativePath}.`);
        }
    }
}
...
for (const [relativePath, approvedHash] of Object.entries(manifest.protectedFiles || {})) {
    if (!exists(relativePath) || sha256(relativePath) !== approvedHash) {
        throw new Error(`[EC-ENGAGEMENT-PRIORITY-V43] alteração não autorizada em ${relativePath}.`);
    }
}

console.log('[EC-ENGAGEMENT-PRIORITY-V43] painel, fila e confirmação local 2/3 congelados e ativos.');
