...
const parentManifestPath = 'docs/freeze/panel-client-search-v41-20260822.json';
const parentManifestSha256 = 'a5f6c2a44621e3887f23b88384eea2ae3612fef54501b8a99aef5cb3201a94f7';
const manifestPath = 'docs/freeze/ec-engagement-command-reply-v42-20260822.json';
const lineageSpecs = [
    ...
    [parentManifestPath, 'panel-client-search-v41-20260822', parentManifestSha256]
...
const declaredParentOverrides = [
    ...
    'public/qr.html',
    'scripts/assert-panel-client-search-activation-approved-v41.mjs',
...
const newProtectedFiles = [
    'approved_freezes/APPROVED_EC_ENGAGEMENT_COMMAND_REPLY_V42_20260822.txt',
    'docs/EC_ENGAGEMENT_COMMAND_REPLY_FREEZE_V42_20260822.md',
    'public/panel-intelligence/ec-engagement-panel-v42.js',
    'scripts/assert-ec-engagement-command-reply-activation-approved-v42.mjs',
    'scripts/guard-ec-engagement-command-reply-v42.mjs',
    'src/services/ecEngagementCommandReplyFreezeRuntimeGuardV42.js',
    'tests/ec-engagement-command-reply-v42.test.mjs'
];
...
for (const spec of lineageSpecs) {
    if (!exists(spec.path) || sha256(spec.path) !== spec.sha256) {
        throw new Error(`[EC-ENGAGEMENT-COMMAND-REPLY-V42] manifesto ancestral divergente: ${spec.path}`);
    }
}
if (!exists(manifestPath)) {
    throw new Error('[EC-ENGAGEMENT-COMMAND-REPLY-V42] manifesto V42 ausente; startup bloqueado.');
}
...
if (
    parent.freezeId !== 'panel-client-search-v41-20260822'
    || manifest.freezeId !== 'ec-engagement-command-reply-v42-20260822'
    ...
    || manifest.operatorApproval?.scope !== 'ec_engagement_command_reply_v42'
    || manifest.policy?.storedBucketBeforeHistoricalOrderFallback !== true
    || manifest.policy?.realOperationalOrderRemainsSovereign !== true
    || manifest.policy?.visualWarmupLabelDeduplication !== true
    || manifest.policy?.manualApprovedPassiveAcknowledgements !== true
    || manifest.policy?.passiveTemplatesAskQuestions !== false
    ...
    || manifest.policy?.orderWriteAdded !== false
    ...
    throw new Error('[EC-ENGAGEMENT-COMMAND-REPLY-V42] manifesto ou política inválida; startup bloqueado.');
}

const overrides = new Set(declaredParentOverrides);
for (let index = 0; index < lineage.length; index += 1) {
    const current = lineage[index];
    const spec = lineageSpecs[index];
    if (current.freezeId !== spec.freezeId) {
        throw new Error(`[EC-ENGAGEMENT-COMMAND-REPLY-V42] freeze ancestral inválido: ${spec.path}`);
    }
    if (index > 0 && current.parentFreezeId !== lineage[index - 1].freezeId) {
        throw new Error('[EC-ENGAGEMENT-COMMAND-REPLY-V42] linhagem V28 → V41 divergente.');
    }
    ...
    for (const [relativePath, approvedHash] of Object.entries(current.protectedFiles || {})) {
        ...
        if (!exists(relativePath) || sha256(relativePath) !== approvedHash) {
            throw new Error(`[EC-ENGAGEMENT-COMMAND-REPLY-V42] herança divergente em ${relativePath}.`);
        }
    }
}
...
for (const [relativePath, approvedHash] of Object.entries(manifest.protectedFiles || {})) {
    if (!exists(relativePath) || sha256(relativePath) !== approvedHash) {
        throw new Error(`[EC-ENGAGEMENT-COMMAND-REPLY-V42] alteração não autorizada em ${relativePath}.`);
    }
}

console.log('[EC-ENGAGEMENT-COMMAND-REPLY-V42] comando, fila e resposta local congelados e ativos.');
