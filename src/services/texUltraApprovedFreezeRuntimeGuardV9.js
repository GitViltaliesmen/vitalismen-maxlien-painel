...
const manifestPath = path.join(root, 'docs', 'freeze', 'ec-manual-product-dropi-security-v9-20260815.json');
const parentManifestPath = path.join(root, 'docs', 'freeze', 'ec-manual-product-dropi-hotfix-v8-20260815.json');
...
if (!fs.existsSync(manifestPath)) {
    if (required) {
        throw new Error('[EC-MANUAL-PRODUCT-DROPI-FREEZE-V9] manifesto aprovado ausente; startup bloqueado.');
    }
} else {
    ...
    if (
        ...
        throw new Error('[EC-MANUAL-PRODUCT-DROPI-FREEZE-V9] manifesto invalido; startup bloqueado.');
    }
    ...
    for (const [relativePath, approvedHash] of Object.entries(parentManifest.protectedFiles || {})) {
        ...
        if (!fs.existsSync(path.join(root, relativePath)) || sha256(relativePath) !== approvedHash) {
            throw new Error(`[EC-MANUAL-PRODUCT-DROPI-FREEZE-V9] regressao no freeze v8: ${relativePath}`);
        }
    }
    for (const [relativePath, approvedHash] of Object.entries(manifest.protectedFiles || {})) {
        if (!fs.existsSync(path.join(root, relativePath)) || sha256(relativePath) !== approvedHash) {
            throw new Error(`[EC-MANUAL-PRODUCT-DROPI-FREEZE-V9] alteracao nao autorizada em ${relativePath}; startup bloqueado.`);
        }
    }
    console.log(`[EC-MANUAL-PRODUCT-DROPI-FREEZE-V9] ${manifest.freezeId} verificado no startup.`);
}
