...
const orderLooksClosed = (order = {}, shipment = null) => {
...
export const getOrderDuplicateGuard = async ({
    ...
    const activeDuplicate = orders.find((order) => {
        ...
        ) && !orderLooksClosed(order, shipment);
    });
    ...
    const latestClosed = orders.find((order) => orderLooksClosed(order, shipmentByOrderId.get(order.orderId)));
    if (latestClosed) {
        return {
            allowed: true,
