...
const parentManifestPath = 'docs/freeze/ec-delivered-repurchase-v45-20260822.json';
const parentManifestSha256 = '610708aa94f6d96efc2dc544c7154fbd1c5716662217d76838f00396a93ffa92';
const manifestPath = 'docs/freeze/ec-repurchase-sync-preservation-v46-20260822.json';
const lineageSpecs = [
    ...
    [parentManifestPath, 'ec-delivered-repurchase-v45-20260822', parentManifestSha256]
...
const newProtectedFiles = [
    ...
    'scripts/assert-ec-repurchase-sync-preservation-activation-approved-v46.mjs',
...
for (const spec of lineageSpecs) {
    if (!exists(spec.path) || sha256(spec.path) !== spec.sha256) {
        throw new Error(`[EC-REPURCHASE-SYNC-V46] manifesto ancestral divergente: ${spec.path}`);
    }
}
if (!exists(manifestPath)) {
    throw new Error('[EC-REPURCHASE-SYNC-V46] manifesto V46 ausente; startup bloqueado.');
}
...
if (
    parent.freezeId !== 'ec-delivered-repurchase-v45-20260822'
    || manifest.freezeId !== 'ec-repurchase-sync-preservation-v46-20260822'
    ...
    || manifest.country !== 'EC'
    || manifest.operatorApproval?.status !== 'implementation_activation_and_target_repair_approved'
    || manifest.policy?.preservePreviousOrderId !== true
    || manifest.policy?.preserveEntryReason !== true
    ...
    throw new Error('[EC-REPURCHASE-SYNC-V46] manifesto ou política inválida; startup bloqueado.');
}
...
for (let index = 0; index < lineage.length; index += 1) {
    ...
    if (current.freezeId !== spec.freezeId) {
        throw new Error(`[EC-REPURCHASE-SYNC-V46] freeze ancestral inválido: ${spec.path}`);
    }
    if (index > 0 && current.parentFreezeId !== lineage[index - 1].freezeId) {
        throw new Error('[EC-REPURCHASE-SYNC-V46] linhagem V28 → V45 divergente.');
    }
    ...
    for (const [relativePath, approvedHash] of Object.entries(current.protectedFiles || {})) {
        ...
        if (!exists(relativePath) || sha256(relativePath) !== approvedHash) {
            throw new Error(`[EC-REPURCHASE-SYNC-V46] herança divergente em ${relativePath}.`);
        }
    }
}
...
for (const [relativePath, approvedHash] of Object.entries(manifest.protectedFiles || {})) {
    if (!exists(relativePath) || sha256(relativePath) !== approvedHash) {
        throw new Error(`[EC-REPURCHASE-SYNC-V46] alteração não autorizada em ${relativePath}.`);
    }
}

console.log('[EC-REPURCHASE-SYNC-V46] linhagem da recompra preservada após salvar a ficha.');
