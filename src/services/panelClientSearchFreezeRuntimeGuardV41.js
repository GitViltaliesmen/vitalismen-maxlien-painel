...
const parentManifestPath = 'docs/freeze/ec-engagement-internal-bucket-v40-20260822.json';
const parentManifestSha256 = '535541c6fbeb55c6722fe1bd1a80766d55e6533592e4f7ea8b7458ed877765c6';
const manifestPath = 'docs/freeze/panel-client-search-v41-20260822.json';
const lineageSpecs = [
    ...
    [parentManifestPath, 'ec-engagement-internal-bucket-v40-20260822', parentManifestSha256]
...
const newProtectedFiles = [
    'approved_freezes/APPROVED_PANEL_CLIENT_SEARCH_V41_20260822.txt',
    'docs/PANEL_CLIENT_SEARCH_FREEZE_V41_20260822.md',
...
for (const spec of lineageSpecs) {
    if (!exists(spec.path) || sha256(spec.path) !== spec.sha256) {
        throw new Error(`[PANEL-CLIENT-SEARCH-V41] manifesto ancestral divergente: ${spec.path}`);
    }
}
if (!exists(manifestPath)) {
    throw new Error('[PANEL-CLIENT-SEARCH-V41] manifesto V41 ausente; startup bloqueado.');
}
...
if (
    parent.freezeId !== 'ec-engagement-internal-bucket-v40-20260822'
    || manifest.freezeId !== 'panel-client-search-v41-20260822'
    ...
    || manifest.operatorApproval?.scope !== 'panel_client_identity_search_v41'
    || manifest.policy?.identityFieldsOnly !== true
    || manifest.policy?.phoneSuffixSearch !== true
    || manifest.policy?.minimumPhoneDigits !== 3
    || manifest.policy?.searchBypassesPresentationFilters !== true
    || manifest.policy?.messageContentSearch !== false
    || manifest.policy?.databaseWriteAdded !== false
    || manifest.policy?.outboundMessageAdded !== false
    ...
    throw new Error('[PANEL-CLIENT-SEARCH-V41] manifesto ou política inválida; startup bloqueado.');
}
...
for (let index = 0; index < lineage.length; index += 1) {
    ...
    if (current.freezeId !== spec.freezeId) {
        throw new Error(`[PANEL-CLIENT-SEARCH-V41] freeze ancestral inválido: ${spec.path}`);
    }
    if (index > 0 && current.parentFreezeId !== lineage[index - 1].freezeId) {
        throw new Error('[PANEL-CLIENT-SEARCH-V41] linhagem V28 → V40 divergente.');
    }
    ...
    for (const [relativePath, approvedHash] of Object.entries(current.protectedFiles || {})) {
        ...
        if (!exists(relativePath) || sha256(relativePath) !== approvedHash) {
            throw new Error(`[PANEL-CLIENT-SEARCH-V41] herança divergente em ${relativePath}.`);
        }
    }
}
...
for (const [relativePath, approvedHash] of Object.entries(manifest.protectedFiles || {})) {
    if (!exists(relativePath) || sha256(relativePath) !== approvedHash) {
        throw new Error(`[PANEL-CLIENT-SEARCH-V41] alteração não autorizada em ${relativePath}.`);
    }
}

console.log('[PANEL-CLIENT-SEARCH-V41] busca por identidade congelada e ativa.');
