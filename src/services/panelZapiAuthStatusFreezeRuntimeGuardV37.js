...
const manifestRelativePath = 'docs/freeze/panel-zapi-auth-status-v37-20260822.json';
const ancestorSpecs = [
...
const newProtectedFiles = [
    'docs/PANEL_ZAPI_AUTH_STATUS_FREEZE_V37_20260822.md',
    'scripts/assert-panel-zapi-auth-status-activation-approved-v37.mjs',
    'scripts/guard-panel-zapi-auth-status-v37.mjs',
    'src/services/panelZapiAuthStatusFreezeRuntimeGuardV37.js',
    'tests/panel-zapi-auth-status-v37.test.mjs'
];
...
for (const spec of ancestorSpecs) {
    if (!exists(spec.path) || sha256(spec.path) !== spec.sha256) {
        throw new Error(`[PANEL-ZAPI-AUTH-STATUS-V37] manifesto ancestral divergente: ${spec.path}`);
    }
}
if (!exists(manifestRelativePath)) {
    throw new Error('[PANEL-ZAPI-AUTH-STATUS-V37] manifesto V37 ausente; startup bloqueado.');
}
...
for (let index = 0; index < ancestors.length; index += 1) {
    if (ancestors[index].freezeId !== ancestorSpecs[index].freezeId) {
        throw new Error(`[PANEL-ZAPI-AUTH-STATUS-V37] freeze ancestral inválido: ${ancestorSpecs[index].path}`);
    }
    if (index > 0 && ancestors[index].parentFreezeId !== ancestors[index - 1].freezeId) {
        throw new Error('[PANEL-ZAPI-AUTH-STATUS-V37] linhagem ancestral divergente.');
    }
}
if (
    ...
    throw new Error('[PANEL-ZAPI-AUTH-STATUS-V37] linhagem V28 → V37 divergente.');
}
...
if (
    manifest.freezeId !== 'panel-zapi-auth-status-v37-20260822'
    || manifest.status !== 'activation_approved'
    || manifest.publicationStatus !== 'authorized_for_draft_pr_and_controlled_activation'
    || manifest.country !== 'EC'
    || manifest.baseProductionSha !== '65e84ad1534aebbaaef09cd1460c4124aab22127'
    || manifest.operatorApproval?.status !== 'approved_in_thread'
    || manifest.operatorApproval?.approvedAt !== '2026-08-22T05:19:15Z'
    ...
    || JSON.stringify(manifest.operatorApproval?.constraints || []) !== JSON.stringify([
        'authenticate_before_status_fetch',
        'no_protected_status_fetch_without_panel_token',
        'keep_zapi_status_route_authenticated',
        'signed_out_state_is_neutral',
        'expired_session_uses_human_message',
        'no_zapi_credentials_or_instance_changes',
        'no_bot_funnel_media_order_or_post_sale_changes',
        'no_real_whatsapp_sends_at_freeze'
    ])
    || manifest.policy?.zapiStatusRouteAuthenticated !== true
    || manifest.policy?.bootstrapAuthBeforeStatus !== true
    || manifest.policy?.anonymousStatusFetch !== false
    || manifest.policy?.signedOutStatusText !== 'Faça login para consultar a conexão'
    || manifest.policy?.expiredSessionText !== 'Sessão expirada. Entre novamente.'
    || manifest.policy?.zapiConfigurationChanged !== false
    || manifest.policy?.officialPhoneChanged !== false
    ...
    throw new Error('[PANEL-ZAPI-AUTH-STATUS-V37] manifesto ou política inválida; startup bloqueado.');
}
...
for (let index = 0; index < ancestors.length; index += 1) {
    ...
    for (const [relativePath, approvedHash] of Object.entries(ancestors[index].protectedFiles || {})) {
        ...
        if (!exists(relativePath) || sha256(relativePath) !== approvedHash) {
            throw new Error(`[PANEL-ZAPI-AUTH-STATUS-V37] herança divergente em ${relativePath}.`);
        }
    }
}
...
for (const [relativePath, approvedHash] of Object.entries(manifest.protectedFiles || {})) {
    if (!exists(relativePath) || sha256(relativePath) !== approvedHash) {
        throw new Error(`[PANEL-ZAPI-AUTH-STATUS-V37] alteração não autorizada em ${relativePath}.`);
    }
}

console.log('[PANEL-ZAPI-AUTH-STATUS-V37] autenticação antes do status; Z-API, painel e freezes V28–V36 preservados.');
