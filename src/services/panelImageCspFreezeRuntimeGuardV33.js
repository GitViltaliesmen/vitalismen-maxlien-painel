...
const manifestRelativePath = 'docs/freeze/panel-image-csp-blob-v33-20260821.json';
const ancestorSpecs = [
...
const newProtectedFiles = [
    'docs/PANEL_IMAGE_CSP_BLOB_FREEZE_V33_20260821.md',
    'scripts/assert-panel-image-csp-activation-approved-v33.mjs',
    'scripts/guard-panel-image-csp-v33.mjs',
    'src/services/panelImageCspFreezeRuntimeGuardV33.js',
...
for (const spec of ancestorSpecs) {
    if (!exists(spec.path) || sha256(spec.path) !== spec.sha256) {
        throw new Error(`[PANEL-IMAGE-CSP-V33] manifesto ancestral divergente: ${spec.path}`);
    }
}
if (!exists(manifestRelativePath)) {
    throw new Error('[PANEL-IMAGE-CSP-V33] manifesto V33 ausente; startup bloqueado.');
}
...
for (let index = 0; index < ancestors.length; index += 1) {
    if (ancestors[index].freezeId !== ancestorSpecs[index].freezeId) {
        throw new Error(`[PANEL-IMAGE-CSP-V33] freeze ancestral inválido: ${ancestorSpecs[index].path}`);
    }
    if (index > 0 && ancestors[index].parentFreezeId !== ancestors[index - 1].freezeId) {
        throw new Error('[PANEL-IMAGE-CSP-V33] linhagem ancestral divergente.');
    }
}
if (
    ...
    throw new Error('[PANEL-IMAGE-CSP-V33] linhagem V28 → V33 divergente.');
}
...
if (
    manifest.freezeId !== 'panel-image-csp-blob-v33-20260821'
    || manifest.status !== 'activation_approved'
    || manifest.publicationStatus !== 'authorized_for_draft_pr_and_controlled_activation'
    || manifest.country !== 'EC'
    || manifest.baseProductionSha !== '14ad0a797c10a7133235b75509c00eb6b57777ee'
    || manifest.operatorApproval?.status !== 'approved_in_thread'
    || manifest.operatorApproval?.approvedAt !== '2026-08-21T22:43:28Z'
    || manifest.operatorApproval?.scope !== 'fix_authenticated_inbound_images_in_panel_v33'
    || JSON.stringify(manifest.operatorApproval?.constraints || []) !== JSON.stringify([
        'img_src_add_blob_only',
        'preserve_authenticated_media_endpoint',
        'preserve_zapi_and_phone_policy',
        'no_whatsapp_sends',
        'no_commercial_flow_changes'
    ])
    || manifest.policy?.authenticatedImageBlobAllowed !== true
    || manifest.policy?.authenticatedMediaEndpointPreserved !== true
    || manifest.policy?.cspChangeLimitedToImgBlob !== true
    || manifest.policy?.zapiAndPhonePolicyPreserved !== true
    ...
    throw new Error('[PANEL-IMAGE-CSP-V33] manifesto ou política inválida; startup bloqueado.');
}
...
for (let index = 0; index < ancestors.length; index += 1) {
    ...
    for (const [relativePath, approvedHash] of Object.entries(ancestors[index].protectedFiles || {})) {
        ...
        if (!exists(relativePath) || sha256(relativePath) !== approvedHash) {
            throw new Error(`[PANEL-IMAGE-CSP-V33] herança divergente em ${relativePath}.`);
        }
    }
}
...
for (const [relativePath, approvedHash] of Object.entries(manifest.protectedFiles || {})) {
    if (!exists(relativePath) || sha256(relativePath) !== approvedHash) {
        throw new Error(`[PANEL-IMAGE-CSP-V33] alteração não autorizada em ${relativePath}.`);
    }
}

console.log('[PANEL-IMAGE-CSP-V33] blob autenticado de imagem permitido; CSP e V32 preservados.');
