...
const manifestRelativePath = 'docs/freeze/ec-product-ingredients-v35-20260822.json';
const ancestorSpecs = [
...
const newProtectedFiles = [
    'docs/EC_PRODUCT_INGREDIENTS_FAQ_FREEZE_V35_20260822.md',
    'docs/FUNIL_ATENDIMENTO_FECHAMENTO.md',
    'scripts/assert-ec-product-ingredients-activation-approved-v35.mjs',
    ...
    'tests/ec-product-ingredients-v35.test.mjs'
];
...
for (const spec of ancestorSpecs) {
    if (!exists(spec.path) || sha256(spec.path) !== spec.sha256) {
        throw new Error(`[EC-PRODUCT-INGREDIENTS-V35] manifesto ancestral divergente: ${spec.path}`);
    }
}
if (!exists(manifestRelativePath)) {
    throw new Error('[EC-PRODUCT-INGREDIENTS-V35] manifesto V35 ausente; startup bloqueado.');
}
...
for (let index = 0; index < ancestors.length; index += 1) {
    if (ancestors[index].freezeId !== ancestorSpecs[index].freezeId) {
        throw new Error(`[EC-PRODUCT-INGREDIENTS-V35] freeze ancestral inválido: ${ancestorSpecs[index].path}`);
    }
    if (index > 0 && ancestors[index].parentFreezeId !== ancestors[index - 1].freezeId) {
        throw new Error('[EC-PRODUCT-INGREDIENTS-V35] linhagem ancestral divergente.');
    }
}
if (
    ...
    throw new Error('[EC-PRODUCT-INGREDIENTS-V35] linhagem V28 → V35 divergente.');
}
...
if (
    manifest.freezeId !== 'ec-product-ingredients-v35-20260822'
    || manifest.status !== 'activation_approved'
    || manifest.publicationStatus !== 'authorized_for_draft_pr_and_controlled_activation'
    || manifest.country !== 'EC'
    || manifest.baseProductionSha !== '1ccbfcfcda2579d4e0d806174cdc575964558316'
    || manifest.operatorApproval?.status !== 'approved_in_thread'
    || manifest.operatorApproval?.approvedAt !== '2026-08-22T03:17:51Z'
    ...
    || JSON.stringify(manifest.operatorApproval?.constraints || []) !== JSON.stringify([
        'product_scoped_text_only',
        'no_product_route_switch',
        'medical_context_handoff_preserved',
        'persistent_lock_and_cooldown',
        'no_media_or_price_changes',
        'no_real_whatsapp_sends_at_freeze'
    ])
    || JSON.stringify(manifest.policy?.productKeys || []) !== JSON.stringify(['tex_ultra_ec', 'nitrix_ec', 'vit_power_ec'])
    || JSON.stringify(manifest.policy?.ingredients || {}) !== JSON.stringify({
        tex_ultra_ec: ['maca peruana', 'Tribulus terrestris', 'catuaba', 'marapuama', 'zinc', 'magnesio'],
        nitrix_ec: ['fenogreco (fenugreek)', 'Tribulus terrestris', 'ginseng Panax (ginseng rojo coreano)', 'ashwagandha', 'Ginkgo biloba', 'L-arginina'],
        vit_power_ec: ['borojó', 'chontaduro', 'noni', 'L-arginina', 'maca', 'guaraná', 'vitaminas']
    })
    || manifest.policy?.deterministicTextOnly !== true
    || manifest.policy?.activeProductRequired !== true
    || manifest.policy?.crossProductReplyBlocked !== true
    ...
    throw new Error('[EC-PRODUCT-INGREDIENTS-V35] manifesto ou política inválida; startup bloqueado.');
}
...
for (let index = 0; index < ancestors.length; index += 1) {
    ...
    for (const [relativePath, approvedHash] of Object.entries(ancestors[index].protectedFiles || {})) {
        ...
        if (!exists(relativePath) || sha256(relativePath) !== approvedHash) {
            throw new Error(`[EC-PRODUCT-INGREDIENTS-V35] herança divergente em ${relativePath}.`);
        }
    }
}
...
for (const [relativePath, approvedHash] of Object.entries(manifest.protectedFiles || {})) {
    if (!exists(relativePath) || sha256(relativePath) !== approvedHash) {
        throw new Error(`[EC-PRODUCT-INGREDIENTS-V35] alteração não autorizada em ${relativePath}.`);
    }
}

console.log('[EC-PRODUCT-INGREDIENTS-V35] FAQ isolada por produto; V28–V34 preservadas.');
