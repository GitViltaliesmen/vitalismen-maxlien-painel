...
const manifestRelativePath = 'docs/freeze/meta-purchase-panel-linkage-v19-20260817.json';
const parentRelativePath = 'docs/freeze/dropi-automatic-submit-reliability-v18-20260817.json';
...
const verifyProtectedFiles = (manifest, skipped = new Set()) => {
    for (const [relativePath, approvedHash] of Object.entries(manifest.protectedFiles || {})) {
        ...
        if (!fs.existsSync(path.join(root, relativePath)) || sha256(relativePath) !== approvedHash) {
            throw new Error(`[META-PURCHASE-PANEL-LINKAGE-V19] alteracao nao autorizada em ${relativePath}; startup bloqueado.`);
        }
    }
};
...
if (!fs.existsSync(manifestPath)) {
    if (required) throw new Error('[META-PURCHASE-PANEL-LINKAGE-V19] manifesto ausente; startup bloqueado.');
} else {
    ...
    if (
        ...
        || manifest.policy?.metaPurchaseDeliveryChanged !== false
        || manifest.policy?.metaPurchaseResendAdded !== false
        || manifest.policy?.purchaseLockDisplayedByLeadId !== true
        || manifest.policy?.offlineLabelForUnlinkedLead !== false
        ...
        throw new Error('[META-PURCHASE-PANEL-LINKAGE-V19] manifesto ou politica invalida; startup bloqueado.');
    }

    verifyProtectedFiles(parent, new Set(manifest.supersededParentProtectedFiles));
    verifyProtectedFiles(manifest);
    console.log(`[META-PURCHASE-PANEL-LINKAGE-V19] ${manifest.freezeId} verificado no startup.`);
}
