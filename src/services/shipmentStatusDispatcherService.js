...
const candidateQuery = (actions = []) => {
    ...
    const branches = [];
    const shouldRefreshPendingDropi = [
        'guide',
        'in_transit',
        'ready_for_pickup'
    ].some((action) => actionSet.has(action));

    // A Dropi order may be accepted before it receives a guide. Without this
    // branch it never reaches refreshShipmentBeforeDispatch(), so a later
    // "ready at agency" status cannot trigger the WhatsApp notice.
    if (shouldRefreshPendingDropi) {
        const retryBefore = new Date(Date.now() - MIN_MESSAGE_GAP_MS);
        branches.push({
            ...
            $and: [
                {
                    $or: [
                        { 'logistics.lastStatusAt': { $exists: false } },
                        { 'logistics.lastStatusAt': null },
                        { 'logistics.lastStatusAt': { $lte: retryBefore } }
                    ]
                }
            ]
        });
