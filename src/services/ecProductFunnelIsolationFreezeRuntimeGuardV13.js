...
const manifestPath = path.join(root, 'docs', 'freeze', 'ec-product-funnel-isolation-v13-20260815.json');
const parentManifestPath = path.join(root, 'docs', 'freeze', 'ec-manual-product-lead-badge-v12-20260815.json');
...
if (!fs.existsSync(manifestPath)) {
    if (required) throw new Error('[EC-PRODUCT-FUNNEL-ISOLATION-FREEZE-V13] manifesto aprovado ausente; startup bloqueado.');
} else {
    ...
    if (
        ...
        throw new Error('[EC-PRODUCT-FUNNEL-ISOLATION-FREEZE-V13] manifesto invalido; startup bloqueado.');
    }
    ...
    for (const [relativePath, approvedHash] of Object.entries(parentManifest.protectedFiles || {})) {
        ...
        if (!fs.existsSync(path.join(root, relativePath)) || sha256(relativePath) !== approvedHash) {
            throw new Error(`[EC-PRODUCT-FUNNEL-ISOLATION-FREEZE-V13] regressao no freeze v12: ${relativePath}`);
        }
    }
    for (const [relativePath, approvedHash] of Object.entries(manifest.protectedFiles || {})) {
        if (!fs.existsSync(path.join(root, relativePath)) || sha256(relativePath) !== approvedHash) {
            throw new Error(`[EC-PRODUCT-FUNNEL-ISOLATION-FREEZE-V13] alteracao nao autorizada em ${relativePath}; startup bloqueado.`);
        }
    }
    console.log(`[EC-PRODUCT-FUNNEL-ISOLATION-FREEZE-V13] ${manifest.freezeId} verificado no startup.`);
}
