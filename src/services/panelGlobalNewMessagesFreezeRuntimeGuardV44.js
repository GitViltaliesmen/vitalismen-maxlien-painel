...
const parentManifestPath = 'docs/freeze/ec-engagement-priority-v43-20260822.json';
const parentManifestSha256 = 'ab4cecc831102ab1257f0f37b6ef8730aaab5a57e63f7759fcdcf0622ffe6034';
const manifestPath = 'docs/freeze/panel-global-new-messages-v44-20260822.json';
const lineageSpecs = [
    ...
    [parentManifestPath, 'ec-engagement-priority-v43-20260822', parentManifestSha256]
...
const newProtectedFiles = [
    'approved_freezes/APPROVED_PANEL_GLOBAL_NEW_MESSAGES_V44_20260822.txt',
    'docs/PANEL_GLOBAL_NEW_MESSAGES_FREEZE_V44_20260822.md',
    'public/panel-intelligence/panel-global-new-messages-v44.js',
    'scripts/assert-panel-global-new-messages-activation-approved-v44.mjs',
    'scripts/guard-panel-global-new-messages-v44.mjs',
    'src/services/panelGlobalNewMessagesFreezeRuntimeGuardV44.js',
    'tests/panel-global-new-messages-v44.test.mjs'
];
...
for (const spec of lineageSpecs) {
    if (!exists(spec.path) || sha256(spec.path) !== spec.sha256) {
        throw new Error(`[PANEL-GLOBAL-NEW-MESSAGES-V44] manifesto ancestral divergente: ${spec.path}`);
    }
}
if (!exists(manifestPath)) {
    throw new Error('[PANEL-GLOBAL-NEW-MESSAGES-V44] manifesto V44 ausente; startup bloqueado.');
}
...
if (
    parent.freezeId !== 'ec-engagement-priority-v43-20260822'
    || manifest.freezeId !== 'panel-global-new-messages-v44-20260822'
    ...
    || manifest.operatorApproval?.scope !== 'panel_global_new_messages_v44'
    || manifest.policy?.globalCommercialNewMessages !== true
    || manifest.policy?.engagementExcludedFromNewMessages !== true
    || manifest.policy?.defaultReturnBucket !== 'attendance'
    || manifest.policy?.databaseWriteAdded !== false
    ...
    throw new Error('[PANEL-GLOBAL-NEW-MESSAGES-V44] manifesto ou política inválida; startup bloqueado.');
}
...
for (let index = 0; index < lineage.length; index += 1) {
    ...
    if (current.freezeId !== spec.freezeId) {
        throw new Error(`[PANEL-GLOBAL-NEW-MESSAGES-V44] freeze ancestral inválido: ${spec.path}`);
    }
    if (index > 0 && current.parentFreezeId !== lineage[index - 1].freezeId) {
        throw new Error('[PANEL-GLOBAL-NEW-MESSAGES-V44] linhagem V28 → V43 divergente.');
    }
    ...
    for (const [relativePath, approvedHash] of Object.entries(current.protectedFiles || {})) {
        ...
        if (!exists(relativePath) || sha256(relativePath) !== approvedHash) {
            throw new Error(`[PANEL-GLOBAL-NEW-MESSAGES-V44] herança divergente em ${relativePath}.`);
        }
    }
}
...
for (const [relativePath, approvedHash] of Object.entries(manifest.protectedFiles || {})) {
    if (!exists(relativePath) || sha256(relativePath) !== approvedHash) {
        throw new Error(`[PANEL-GLOBAL-NEW-MESSAGES-V44] alteração não autorizada em ${relativePath}.`);
    }
}

console.log('[PANEL-GLOBAL-NEW-MESSAGES-V44] contador e lista global de Novas congelados e ativos.');
