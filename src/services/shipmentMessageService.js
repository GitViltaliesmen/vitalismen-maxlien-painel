...
export const buildPickupBonusText = (shipment = null) => {
    ...
    const product = resolveEcuadorProductInfo(shipment || {});
    if (product.key === ECUADOR_PRODUCTS.texUltra.key) {
        return withSaveContactReminder(chooseStableVariant(BONUS_TEXT_VARIANTS, shipment?.orderId || shipment?.logistics?.trackingNumber || 'tex_ultra_ec'));
    }
...
export const buildRefillReminderText = (shipment) => {
    ...
    const product = resolveEcuadorProductInfo(shipment || {});
    if (product.key === ECUADOR_PRODUCTS.texUltra.key) {
        return withSaveContactReminder(`Hola, senor. Vi que compro ${unitsLabel} de Tex Ultra. Si desea continuar su tratamiento, le ayudo a revisar una oferta de recompra por este canal oficial.`);
    }
...
const postSaleAudioName = (shipment, kind) => {
    const product = resolveEcuadorProductInfo(shipment || {});
    if (product.key === ECUADOR_PRODUCTS.texUltra.key) {
        return TEX_ULTRA_EC_PRODUCT_PROFILE.postSale[kind] || '';
    }
    return {
        thankYouAudioName: 'OBRIGADO_PAGOU',
        howToUseAudioName: 'COMO_SE_TOMA_VIT_POWER',
        refillAudioName: 'TEMPO_RESULTADO_VIT_POWER'
    }[kind] || '';
};
...
export const notifyPickupBonus = async (shipment) => {
    ...
    const thankYouAudioName = postSaleAudioName(shipment, 'thankYouAudioName');
    const thankYouAudioPath = thankYouAudioName
        ? await resolveCountryAudio({ country: shipment.country || 'EC', baseName: thankYouAudioName })
        : null;
    ...
    if (!sent) return false;
    const howToUseAudioName = postSaleAudioName(shipment, 'howToUseAudioName');
    const howToUseAudioPath = howToUseAudioName
        ? await resolveCountryAudio({ country: shipment.country || 'EC', baseName: howToUseAudioName })
        : null;
...
export const notifyTreatmentRefillReminder = async (shipment) => {
    ...
    const refillAudioName = postSaleAudioName(shipment, 'refillAudioName');
    const tempoAudioPath = refillAudioName
        ? await resolveCountryAudio({ country: shipment.country || 'EC', baseName: refillAudioName })
        : null;
    const audioSent = tempoAudioPath
        ? await sendShipmentAudioFile(shipment, chatId, tempoAudioPath, {
            ...
            baseName: refillAudioName,
            dedupeValue: `refill_reminder_audio|${refillAudioName}|${shipment.orderId || shipment.logistics?.trackingNumber || ''}`
        })
        : false;
