...
if (!fs.existsSync(manifestPath)) {
    if (required) {
        throw new Error('[TEX-ULTRA-FREEZE-V5] manifesto aprovado ausente; startup bloqueado.');
    }
} else {
    const manifest = JSON.parse(fs.readFileSync(manifestPath, 'utf8'));
    if (manifest.status !== 'approved_frozen' || manifest.country !== 'EC' || manifest.productKey !== 'tex_ultra_ec') {
        throw new Error('[TEX-ULTRA-FREEZE-V5] manifesto invalido; startup bloqueado.');
    }
    for (const [relativePath, approvedHash] of Object.entries(manifest.protectedFiles || {})) {
        const absolutePath = path.join(root, relativePath);
        if (!fs.existsSync(absolutePath)) {
            throw new Error(`[TEX-ULTRA-FREEZE-V5] arquivo protegido ausente: ${relativePath}`);
        }
        const actualHash = crypto.createHash('sha256').update(fs.readFileSync(absolutePath)).digest('hex');
        if (actualHash !== approvedHash) {
            throw new Error(`[TEX-ULTRA-FREEZE-V5] alteracao nao autorizada em ${relativePath}; startup bloqueado.`);
        }
    }
    console.log(`[TEX-ULTRA-FREEZE-V5] ${manifest.freezeId} verificado no startup.`);
}
