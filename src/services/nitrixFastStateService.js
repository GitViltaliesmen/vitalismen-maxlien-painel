...
const commitDispatchedFlowIfCurrent = async ({ state, flow, jobIndex, extra = {}, sentAt = null }) => {
    const memory = { ...productMemory(state), fastState: flow, ...extra };
    const startedAt = new Date(flow.startedAt);
    const sentAtDate = sentAt ? new Date(sentAt) : null;
    const openingWasSent = Boolean(sentAtDate)
        && String(flow?.jobs?.[jobIndex]?.id || '') === 'opening_text';
    const nextTags = openingWasSent
        ? (Array.isArray(state.tags) ? state.tags : []).filter((tag) => tag !== 'AGUARDANDO_ATENDIMENTO')
    ...
    // A VSL só deve ficar em atenção até a primeira saída aprovada. Sem este
    // campo, mensagens já enviadas podem parecer "sem atendimento" no painel.
    if (sentAtDate) set.lastOutboundAt = sentAtDate;
    if (openingWasSent) set['metadata.vslInitialResponseAt'] = sentAtDate.toISOString();
...
const nitrixPostSaleAutomationEnabled = () => String(
    process.env.NITRIX_POST_SALE_AUTOMATION_ENABLED || 'false'
).toLowerCase() === 'true';

const normalizedPostSaleStatus = (value = '') => String(value || '')
    .trim()
    .toLowerCase();

export const isNitrixConfirmedPostSaleState = (state = {}) => {
    if (!nitrixPostSaleAutomationEnabled()) return false;
    const metadata = state.metadata || {};
    const draft = metadata.customerDraft || {};
    const status = normalizedPostSaleStatus(
        draft.status || metadata.orderStatus || metadata.status
    );
    const confirmedStatuses = new Set([
        'confirmado', 'confirmed', 'processing', 'shipped', 'delivered', 'entregue'
    ]);
    return vslNitrixSourceConfirmed(state)
        && confirmedStatuses.has(status)
        && metadata.nitrixPostSaleManualHold !== true;
};
...
const normalizeNitrixVslEntryText = (text = '') => String(text || '')
    .normalize('NFD')
    .replace(/[\u0300-\u036f]/g, '')
    .toLowerCase()
    .replace(/[^\p{L}\p{N}]+/gu, ' ')
...
// As duas frases A/B sao a origem publica atual. A terceira e somente uma
// contingencia para a frase de uma pagina antiga, evitando que ela bloqueie a
// acolhida em espanhol enquanto a aba antiga ainda existir no aparelho.
const knownNitrixVslEntryTexts = new Set([
    'Hola, quiero saber mas sobre Nitrix Oxide.',
    'Hola, deseo recibir mas informacion sobre el producto.',
    'Olá! Tenho interesse e queria mais informações, por favor.'
].map(normalizeNitrixVslEntryText));

const isKnownNitrixVslEntryText = (text = '') => (
    knownNitrixVslEntryTexts.has(normalizeNitrixVslEntryText(text))
);
...
const initialVslGreeting = (text = '') => (
    ...
    || looksLikeName(text)
    || isKnownNitrixVslEntryText(text)
);
...
const isPostSaleDeliveryQuestion = (text = '') => /\b(guia|entrega|envio|envi[oó]|cuando llega|cu[aá]ndo llega|retiro|retirar|agencia|servientrega|estado|estatus|status)\b|\bmi pedido\b/i.test(String(text || ''));
const isPostSaleThanks = (text = '') => /\b(gracias|muchas gracias|perfecto|listo|correcto)\b/i.test(String(text || ''));

const saveNitrixPostSaleMemory = async (state, values = {}) => {
    const metadata = state.metadata || {};
    const agentMemory = metadata.perAgentMemory?.[AGENT_KEY] || {};
    state.metadata = {
        ...metadata,
        perAgentMemory: {
            ...
            [AGENT_KEY]: {
                ...agentMemory,
                postSale: {
                    ...(agentMemory.postSale || {}),
                    ...values,
                    updatedAt: nowIso()
                },
                lastFunnelStage: 'nitrix_post_sale'
            }
        },
        lastKnownFunnelStage: 'nitrix_post_sale'
    };
    state.markModified('metadata');
    await state.save();
};

const handoffNitrixToHuman = async ({ state, reason, note }) => {
    const current = clone(flowOf(state));
    if (current?.status === 'running') cancelPendingJobs(current, reason);
    state.human = {
        ...(state.human || {}),
        mode: 'manual',
        pausedUntil: null,
    ...
    state.tags = [...new Set([...(state.tags || []), 'NITRIX_EC', 'AGUARDANDO_ATENDIMENTO'])];
    if (current?.startedAt) {
        await updateFlow(state, current, { lastFunnelStage: reason });
        return;
    }
    await state.save();
};

export const handleNitrixPostSaleInbound = async ({ state, text, sessionId = null } = {}) => {
    if (!state || !isNitrixConfirmedPostSaleState(state)) return { handled: false };
    const memory = productMemory(state);
    const postSale = memory.postSale || {};
    const flow = {
        ...(clone(flowOf(state)) || {}),
        sessionId: sessionId || flowOf(state)?.sessionId || null,
        usageVariantIndex: Number(postSale.usageVariantIndex || 0)
    };

    if (asksUsage(text)) {
        const audioName = NITRIX_EC_PRODUCT_PROFILE.usage.approvedAudioName;
        const audioPath = audioName ? await resolveCountryAudio({ country: 'EC', baseName: audioName }) : '';
        const audioSent = Boolean(audioPath) && await sendAudio(safeChatId(state), audioPath, true, {
            sessionId: flow.sessionId,
            country: 'EC',
            outboundContext: 'nitrix_post_sale_usage',
            skipAfterSendPacing: true
        });
        if (audioSent) {
            await recordOutbound({ chatId: safeChatId(state), peerPhone: state.phoneDigits, body: `[AUDIO] ${audioName}`, type: 'audio', mediaPath: audioPath, sessionId: flow.sessionId });
        ...
        const companion = nextUsageCompanion(flow);
        const textSent = companion
            ? await sendPlainText({ state, flow, text: companion, context: 'nitrix_post_sale_usage' })
            : false;
        await saveNitrixPostSaleMemory(state, {
            usageAudioSentAt: audioSent ? nowIso() : postSale.usageAudioSentAt || '',
            usageTextSentAt: textSent ? nowIso() : postSale.usageTextSentAt || '',
            usageVariantIndex: flow.usageVariantIndex
        });
        return { handled: Boolean(audioSent || textSent), kind: 'usage' };
    }

    if (asksGuarantee(text)) {
        const sent = await sendPlainText({ state, flow, text: NITRIX_EC_PRODUCT_PROFILE.guarantee.text, context: 'nitrix_post_sale_guarantee' });
        await saveNitrixPostSaleMemory(state, { guaranteeSentAt: sent ? nowIso() : postSale.guaranteeSentAt || '' });
        return { handled: sent, kind: 'guarantee' };
    }

    if (isPostSaleDeliveryQuestion(text)) {
        const sent = await sendPlainText({
            state,
            flow,
            text: 'Su pedido ya quedó registrado por el canal oficial. Para confirmar guía, agencia o fecha de entrega, el equipo revisa el panel y le responde por este mismo chat.',
            context: 'nitrix_post_sale_delivery'
        });
        await saveNitrixPostSaleMemory(state, { deliveryGuidanceSentAt: sent ? nowIso() : postSale.deliveryGuidanceSentAt || '' });
        return { handled: sent, kind: 'delivery' };
    }

    if (isPostSaleThanks(text)) {
        const sent = await sendPlainText({ state, flow, text: 'Con gusto. Seguimos acompañándole por este canal oficial si necesita orientación sobre su pedido o el uso de Nitrix.', context: 'nitrix_post_sale_thanks' });
        await saveNitrixPostSaleMemory(state, { thanksSentAt: sent ? nowIso() : postSale.thanksSentAt || '' });
        return { handled: sent, kind: 'thanks' };
    }

    await handoffNitrixToHuman({
        state,
        reason: 'nitrix_post_sale_handoff',
        note: 'Pós-venda Nitrix: dúvida fora de uso, garantia ou acompanhamento. Atendimento humano assumiu sem reabrir venda.'
    });
    state.metadata = { ...(state.metadata || {}), nitrixPostSaleManualHold: true };
    state.markModified('metadata');
    await state.save();
    return { handled: true, kind: 'human_handoff' };
};
...
const processState = async (state, now) => {
    ...
    // O texto de acolhida e a primeira confirmacao de atendimento: ele nao
    // pode ficar atras de audio/imagem de outro cliente. A cadencia global
    // permanece obrigatoria para as midias seguintes.
    const queueSlot = job.id === 'opening_text'
        ? { allowed: true, gapMs: 0, availableAt: now.getTime() }
        : reserveScheduledMediaSlot(now.getTime());
...
export const handleNitrixFastStateInbound = async ({ contactStateId, inboundText, sessionId = null } = {}) => {
    ...
    const postSale = await handleNitrixPostSaleInbound({ state, text: inboundText, sessionId });
    if (postSale.handled) return true;
    const current = clone(flowOf(state));
    if (current?.startedAt) {
        if (['two_audio_only', 'full_sequence'].includes(String(current.entryLayer || ''))) {
            await handoffNitrixToHuman({
                state,
                reason: 'nitrix_entry_handoff',
                note: current.entryLayer === 'full_sequence'
                    ? 'A sequência inicial Nitrix foi interrompida pela resposta do cliente. Venda e fechamento seguem exclusivamente com atendimento humano.'
                    : 'Os dois áudios iniciais Nitrix foram enviados. Venda e fechamento seguem exclusivamente com atendimento humano.'
            });
            return true;
        }
