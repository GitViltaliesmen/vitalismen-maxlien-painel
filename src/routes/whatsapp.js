...
import { panelHandledThroughSeconds, panelLastReadMarkerSeconds, panelReadIdentityQuery } from '../services/panelReadStateService.js';
import {
...
router.get('/chats', async (req, res) => {
    try {
        ...
        if (fastMode) {
            ...
            inboundMessageTimesByKey.forEach((inboundTimes, conversationKey) => {
                ...
                const handledThrough = panelHandledThroughSeconds({
                    ...
                    readThrough: chat ? fastReadMarkerForChat(chat) : 0,
                    lastOutboundAt
                });
        ...
        const enrichedChats = await Promise.all(allChats.map(async (c) => {
            ...
            const handledThrough = panelHandledThroughSeconds({ states: contactStates, lastOutboundAt });
