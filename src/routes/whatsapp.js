...
import { editZapiText, sendZapiDocument } from '../services/zapiClient.js';
...
const isVisiblePanelMessage = (message = {}) => (
    (String(message.provider || '') !== 'zapi_chat_watchdog' || Boolean(String(message.body || '').trim()))
...
// POST /api/whatsapp/messages/:phone/edit
// WhatsApp only accepts edits to a sent text within its short edit window.
// The original local body is retained in editHistory for panel auditability.
router.post('/messages/:phone/edit', authMiddleware, async (req, res) => {
    try {
        const { phone } = req.params;
        const messageId = String(req.body?.messageId || '').trim();
        const body = String(req.body?.message || '').trim();
        if (!messageId || !body) return res.status(400).json({ error: 'messageId e texto atualizado sao obrigatorios' });
        if (body.length > 4096) return res.status(400).json({ error: 'A mensagem pode ter no maximo 4096 caracteres' });
        ...
        const message = await Message.findOne({ _id: messageId, $or: or });
        if (!message) return res.status(404).json({ error: 'Mensagem nao encontrada nesta conversa' });
        if (!message.isFromMe || message.isBot || !['chat', 'text'].includes(String(message.type || '').toLowerCase())) {
            return res.status(403).json({ error: 'So mensagens de texto enviadas manualmente podem ser editadas' });
        }
        if (String(message.provider || '').toLowerCase() !== 'zapi') {
            return res.status(409).json({ error: 'Esta mensagem nao foi enviada pela Z-API e nao pode ser editada por este painel' });
        }

        const providerMessageId = String(
            message.providerMessageId
            || message.providerPayload?.messageId
            || message.providerPayload?.id
            || ''
        ).trim();
        if (!providerMessageId) {
            return res.status(409).json({ error: 'A Z-API nao retornou o ID desta mensagem; nao e possivel editar no WhatsApp' });
        }

        const sentAtMs = Number(message.timestamp || 0) * 1000
            || new Date(message.createdAt || 0).getTime();
        const ageMs = Math.max(0, Date.now() - sentAtMs);
        if (!sentAtMs || ageMs > WHATSAPP_EDIT_WINDOW_MS) {
            return res.status(409).json({ error: 'O WhatsApp permite editar mensagens apenas nos primeiros 15 minutos apos o envio' });
        }
        if (String(message.body || '').trim() === body) {
            return res.json({ success: true, unchanged: true, body: message.body, editedAt: message.editedAt || null });
        }

        const recipientPhone = digitsOnly(message.peerPhone || phone || message.to);
        if (!recipientPhone) return res.status(409).json({ error: 'Telefone do destinatario nao foi localizado para a edicao' });

        const providerResponse = await editZapiText({
            phone: recipientPhone,
            message: body,
            editMessageId: providerMessageId
        });
        const editedAt = new Date();
        const editedBy = req.user?.name || req.user?.email || 'painel';
        const previousBody = String(message.body || '');
        await Message.updateOne({ _id: message._id }, {
            $set: {
                body,
                originalBody: message.originalBody || previousBody,
                editedAt,
                editedBy,
                lastEditProviderPayload: providerResponse
            },
            $inc: { editCount: 1 },
            $push: {
                editHistory: {
        ...
        const detail = error?.response?.data?.message || error?.response?.data?.error || error.message || 'Falha ao editar mensagem no WhatsApp';
        console.error('[ZAPI-EDIT-MESSAGE] falha:', detail);
        res.status(error?.statusCode || error?.response?.status || 500).json({ error: String(detail) });
    }
});
