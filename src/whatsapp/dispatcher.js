...
export const setupDispatcher = (sock, currentSocketId = 'N/A', sessionId = 'default') => {
    sock.ev.on('messages.upsert', async (payload) => {
        ...
        if (!cutoverPolicy.canProcessWebInbound(cutoverPhone, [{
            sessionId,
            ownPhoneDigits: getOwnPhoneDigits(sessionId),
            isReady: true,
            status: 'connected'
        }])) {
            console.log(`[DISPATCHER] inbound Web em observacao, sem processamento | mode=${cutoverPolicy.mode} | session=${sessionId}`);
            return;
        }
