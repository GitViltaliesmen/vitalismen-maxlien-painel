...
import {
    R4_CHECKPOINT_PATH, R4_SUCCESSOR_CHECKPOINT_PATH,
...
assert.equal(process.platform, 'linux', 'R4_SUCCESSOR_AUTHORITY_LINUX_ONLY');
assert.equal(process.getuid?.(), 0, 'R4_SUCCESSOR_AUTHORITY_ROOT_ONLY');
assert.equal(process.argv.length, 2, 'R4_SUCCESSOR_AUTHORITY_NO_ARGUMENTS');
assert.equal(fs.existsSync(R4_SUCCESSOR_CHECKPOINT_PATH), false,
    'R4_SUCCESSOR_AUTHORITY_EXISTS');
...
assert.deepEqual(git('rev-list', '--parents', '-n', '1', 'HEAD').split(' '),
    [commit, R4_PARENT_COMMIT], 'R4_SUCCESSOR_NOT_SINGLE_CHILD');
assert.equal(git('rev-parse', `${R4_PARENT_COMMIT}^{tree}`), R4_PARENT_TREE,
    'R4_SUCCESSOR_PARENT_TREE_CHANGED');
const parent = readAuthorizedCheckpoint();
assert.equal(parent.sha256, R4_PARENT_CHECKPOINT_SHA256);
assert.equal(digest(R4_MANIFEST_PATH), parent.value.r4OperationalManifestSha256,
    'R4_SUCCESSOR_PARENT_MANIFEST_CHANGED');
const manifest = validateR4Manifest(
    readCanonicalJson(path.join(root, R4_SUCCESSOR_MANIFEST_PATH)).value);
const checkpoint = validateCheckpoint({
    ...
    parentR4Commit: R4_PARENT_COMMIT,
    parentR4Tree: R4_PARENT_TREE,
    project: 'MAXLIEN EC — VITALISMEN OFICIAL',
    r4OperationalCommit: commit,
    r4OperationalTree: tree,
    r4OperationalManifestSha256: digest(R4_SUCCESSOR_MANIFEST_PATH),
...
const temporary = `${R4_SUCCESSOR_CHECKPOINT_PATH}.${process.pid}.tmp`;
...
fs.renameSync(temporary, R4_SUCCESSOR_CHECKPOINT_PATH);
fs.chmodSync(R4_SUCCESSOR_CHECKPOINT_PATH, 0o400);
const directory = fs.openSync(path.dirname(R4_SUCCESSOR_CHECKPOINT_PATH), 'r');
try { fs.fsyncSync(directory); } finally { fs.closeSync(directory); }
const verified = readCanonicalJson(R4_SUCCESSOR_CHECKPOINT_PATH);
assert.deepEqual(validateCheckpoint(verified.value), checkpoint);
process.stdout.write(`R4_SUCCESSOR_AUTHORITY=${R4_SUCCESSOR_CHECKPOINT_PATH}\n`);
