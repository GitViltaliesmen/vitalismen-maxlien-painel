...
const v176Context = globalThis.__VITALISMEN_V176_PANEL_CONTACTABLE_PRELEAD_CONTEXT;
...
assert.equal(v176Context?.loaded, true, '[V176] successor_context_not_loaded');
assert.equal(v176Context.freezeId, 'EC_PANEL_CONTACTABLE_PRELEAD_V176_20260917');
...
const v176Filter = panel.slice(
    panel.indexOf('const isUncontactableVslPrelead ='),
    panel.indexOf('const visibleOperationalChats =')
);
assert.match(v176Filter, /chat\.vslPrelead === true/);
assert.match(v176Filter, /chat\.phone \|\| chat\.customerDraft\?\.phone/);
assert.doesNotMatch(v176Filter, /chat\.id/);
assert.match(panel, /if \(isUncontactableVslPrelead\(chat\)\) return false;/);
...
console.log(`EC_PANEL_CONTACTABLE_PRELEAD_V176=PASS manifest_sha256=${v176Context.manifestSha256}`);
