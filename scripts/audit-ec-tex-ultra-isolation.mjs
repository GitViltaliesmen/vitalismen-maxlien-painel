...
assert.match(publicOffer, /2 frascos por 70\.00 USD/);
...
assert.equal(TEX_ULTRA_EC_PRODUCT_PROFILE.bottle.media, '/media/sales/ec/tex_ultra.png');
...
const confirmedPostSale = read('src/services/texUltraConfirmedPostSaleLayerService.js');
assert.match(confirmedPostSale, /AGRADECIMENTO_AGENCIA_DE_ENTREGA/);
assert.match(confirmedPostSale, /BONUS_RETIRADA/);
assert.match(confirmedPostSale, /stepKey: 'thankYouAgency'/);
assert.match(confirmedPostSale, /stepKey: 'pickupBonus'/);
