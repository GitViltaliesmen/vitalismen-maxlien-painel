...
if (!scripts.length || scripts.some(script => !/^[a-z0-9:_-]+$/i.test(script))) {
    throw new Error('V185 npm script ausente ou invalido.');
}
const preload = new URL('./lib/ec-runtime-successor-v185-context.mjs', import.meta.url).href;
...
for (const script of scripts) {
    const result = spawnSync(`${npmExecutable} run ${script}`, [], {
        cwd: process.cwd(),
        env: { ...process.env, NODE_OPTIONS: nodeOptions, V152_B_SHADOW_TEST_CONTEXT: 'true' },
        stdio: 'inherit',
        shell: true
    });
