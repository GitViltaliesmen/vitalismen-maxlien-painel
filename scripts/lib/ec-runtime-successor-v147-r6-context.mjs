...
for (const [file, expected] of Object.entries({ ...manifest.preservedFiles, ...manifest.protectedFiles })) {
    ...
    assert.equal(crypto.createHash('sha256').update(fs.readFileSync(new URL('../../' + file, import.meta.url))).digest('hex'), globalThis.__VITALISMEN_V165_CONTEXT?.protectedFiles?.[file] || globalThis.__VITALISMEN_V147_R6R2_CONTEXT?.protectedFiles?.[file] || expected, '[V147-R6] ' + file);
}
globalThis.__VITALISMEN_V147_R6_CONTEXT = Object.freeze({ loaded: true, freezeId: manifest.freezeId,
    manifestSha256: crypto.createHash('sha256').update(text).digest('hex'), protectedFiles: Object.freeze({ ...manifest.protectedFiles, ...globalThis.__VITALISMEN_V147_R6R2_CONTEXT?.protectedFiles, ...globalThis.__VITALISMEN_V165_CONTEXT?.protectedFiles }) });
