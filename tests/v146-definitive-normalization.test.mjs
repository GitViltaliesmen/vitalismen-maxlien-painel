...
test('V97 carrega V146 e preserva V145 pela cadeia oficial', () => {
    const preload = pathToFileURL(path.join(root, 'scripts/lib/ec-runtime-successor-v97-context.mjs')).href;
    const probe = [
        "const v145=globalThis.__VITALISMEN_V145_R2_CONTEXT",
        "const v146=globalThis.__VITALISMEN_V146_CONTEXT",
        "const overrides=new Set(globalThis.__VITALISMEN_SUCCESSOR_OVERRIDE_FILES||[])",
        "if(!v145?.loaded||!v146?.loaded||!overrides.has('scripts/guard-ec-delivered-repurchase-v45.mjs'))process.exit(2)",
        "console.log('V145_CONTEXT_LOADED=YES')",
        "console.log('V146_CONTEXT_LOADED=YES')",
        "console.log('V146_OVERRIDES_REGISTERED=YES')"
    ].join(';');
    ...
    assert.match(result.stdout, /V146_CONTEXT_LOADED=YES/);
    assert.match(result.stdout, /V146_OVERRIDES_REGISTERED=YES/);
});
