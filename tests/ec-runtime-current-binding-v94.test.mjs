...
test('V94 separa o preload físico do stage do preload estável do PM2', () => {
    ...
    assert.match(helper, /preload_path="\$candidate_dir\/scripts\/lib\/ec-runtime-successor-v96-context\.mjs"/);
    assert.ok(helper.includes(`target_node_options="${EC_RUNTIME_TRANSIENT_RESET_V96_NODE_OPTIONS}"`));
    assert.ok(restart.includes(EC_RUNTIME_TRANSIENT_RESET_V96_NODE_OPTIONS));
    assert.ok(operational.includes(EC_RUNTIME_TRANSIENT_RESET_V96_NODE_OPTIONS));
});
