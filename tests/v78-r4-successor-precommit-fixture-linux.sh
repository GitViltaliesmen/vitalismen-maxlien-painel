...
[[ "$fixture_root" =~ ^/tmp/vitalismen-r4-control-plane\.[A-Za-z0-9]+$ ]] || {
...
parent_release='/opt/vitalismen-automacao/releases/20260925T134302Z_production-20260925-da2983f'
...
[[ ! -e /var/lib/vitalismen-deploy/CHECKPOINT_R4_V78_CONTROL_PLANE_AUTHORITY.json ]] || {
...
const parent = JSON.parse(fs.readFileSync(authority.R4_CHECKPOINT_PATH));
assert.equal(sha(fs.readFileSync(authority.R4_CHECKPOINT_PATH)),
    authority.R4_PARENT_CHECKPOINT_SHA256);
const manifestFile = path.join(root, authority.R4_SUCCESSOR_MANIFEST_PATH);
...
const checkpoint = authority.validateCheckpoint({
    ...
    parentCheckpointSha256: authority.R4_PARENT_CHECKPOINT_SHA256,
    parentR4Commit: authority.R4_PARENT_COMMIT,
    parentR4Tree: authority.R4_PARENT_TREE,
    project: 'MAXLIEN EC — VITALISMEN OFICIAL',
    r4OperationalCommit: commit, r4OperationalTree: tree,
    r4OperationalManifestSha256: digest(authority.R4_SUCCESSOR_MANIFEST_PATH),
...
write(authority.R4_SUCCESSOR_CHECKPOINT_PATH, checkpoint);
const checkpointSha256 = sha(fs.readFileSync(authority.R4_SUCCESSOR_CHECKPOINT_PATH));
const attestation = {
...
export V78_R4_EXPECTED_IDENTITY='R4_SUCCESSOR'
